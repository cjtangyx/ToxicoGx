...
## CREATE SENSITIVITY$PROFILES (LDH only) ##
#Select only for entries where LDH info is present -> LDHpresent
#For LDH-based viability info, all dosages were in uM
LDHpresent<-subset(phenoData,phenoData$LDH != "NA",select=c(samplename, exp_id, group_id, individual_id, drugid, duration, concentration, dose_level, LDH,UID))
#The data.frame finalCombined will start with all the samples that do not have LDH data
finalCombined<-subset(phenoData,is.na(phenoData$LDH),select=c(samplename, exp_id, group_id, individual_id, drugid, duration, concentration, dose_level, LDH,UID))
finalCombined$slope_recomputed<-NA
finalCombined$auc_recomputed<-NA
#Get all unique drug names in LDHpresent (for the for loop later)
uniqueDrugNames<-unique(LDHpresent$drugid)

#Create an empty data frame with all required columns
emptyFrame<-subset(phenoData,select=c(samplename, exp_id, group_id, individual_id, drugid, duration, concentration, dose_level, LDH,UID))
...
for (drug in uniqueDrugNames){ #iterate through each unique drug (set of experiments)
  ...
  drugSamples<-subset(LDHpresent,LDHpresent$drugid == drug) #drugSamples contains all the experiments for one drug
  ...
  if (NROW(samples2_1) >= 3){
    samples2_1$slope_recomputed = computeSlope(samples2_1$concentration,samples2_1$LDH)
    samples2_1$auc_recomputed = computeAUC(samples2_1$concentration, samples2_1$LDH,conc_as_log = FALSE, viability_as_pct = TRUE, area.type = "Actual")
  }
  if (NROW(samples2_2) >= 3){
    samples2_2$slope_recomputed = computeSlope(samples2_2$concentration,samples2_2$LDH)
    samples2_2$auc_recomputed = computeAUC(samples2_2$concentration, samples2_2$LDH,conc_as_log = FALSE, viability_as_pct = TRUE, area.type = "Actual")
  }
  if (NROW(samples8_1) >= 3){
    samples8_1$slope_recomputed = computeSlope(samples8_1$concentration,samples8_1$LDH)
    samples8_1$auc_recomputed = computeAUC(samples8_1$concentration, samples8_1$LDH,conc_as_log = FALSE, viability_as_pct = TRUE, area.type = "Actual")
  }
  if (NROW(samples8_2) >= 3){
    samples8_2$slope_recomputed = computeSlope(samples8_2$concentration,samples8_2$LDH)
    samples8_2$auc_recomputed = computeAUC(samples8_2$concentration, samples8_2$LDH,conc_as_log = FALSE, viability_as_pct = TRUE, area.type = "Actual")
  }
  if (NROW(samples24_1) >= 3){
    samples24_1$slope_recomputed = computeSlope(samples24_1$concentration,samples24_1$LDH)
    samples24_1$auc_recomputed = computeAUC(samples24_1$concentration, samples24_1$LDH,conc_as_log = FALSE, viability_as_pct = TRUE, area.type = "Actual")
  }
  if (NROW(samples24_2) >= 3){
    samples24_2$slope_recomputed = computeSlope(samples24_2$concentration,samples24_2$LDH)
    samples24_2$auc_recomputed = computeAUC(samples24_2$concentration, samples24_2$LDH,conc_as_log = FALSE, viability_as_pct = TRUE, area.type = "Actual")
  }
...
## CREATE SENSITIVITY$RAW (LDH only) ##
...
#Viability Info (& dose info) from phenoData (LDH) & reformatting
allViability <- subset(phenoData, select=c(UID, concentration, LDH,dose_level))
...
#Write objects into rds/ file
saveRDS(sensInfo,"rds/sensitivityInfo_LDH.rds")
saveRDS(sensProf,"rds/sensitivityProfiles_LDH.rds")
saveRDS(sensRaw,"rds/sensitivityRaw_LDH.rds")
