...
CELgenes<-read.csv("data/rownames.csv",sep="\t",header=TRUE,stringsAsFactors = TRUE)
CELgenes<-gsub("_at","",CELgenes$Ensembl_ID)
...
CELnotbiomaRt<-unique(CELgenes)[!(unique(CELgenes) %in% uniqueBiomaRt$gene_id)] #in CELgenes but not in uniqueBiomaRt (59)
...
#Write into file
write.csv(finalFeature,"finalFeature.csv")
